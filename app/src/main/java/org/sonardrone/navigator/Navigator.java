...
public class Navigator {
	...
	public void readResources() {
		String[] intParams = { "resumeFromWp", "load", "rudderAngle"};

		String[] doubleParams = { "k",
		...
		for (int i = 0; i < doubleParams.length; i++) {
			try {
				...
				Log.d(TAG,"Setting field:" + doubleParams[i]);
	...
	void initWaypoints() {
		// polls until waypoints.txt can be read or until waypoints are added from remote		
		while (this.wp.size() == 0 && NavigatorService.operative) {
			...
			try {
				Log.d(TAG,"Waiting or waypoints...");
				Thread.sleep(5000);
			} catch (InterruptedException e) {
			}
		}
